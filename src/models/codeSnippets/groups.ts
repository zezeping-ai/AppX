/** 代码段分组定义（数据层 canonical 来源，modules 仅 re-export） */
export const CODE_SNIPPET_GROUPS = [
  { value: "general", label: "通用", icon: "mdi:tag-outline" },
...
export function iconOfSnippetGroup(group: string): string {
  return CODE_SNIPPET_GROUPS.find((item) => item.value === group)?.icon ?? "mdi:folder-outline";
}
